...
void Application::Init(Display* display, const CommandLine& arguments)
{
    ...
    GUI::SetPlayAudioCallback(display, [](void* user, const char* filename, float volume)
    {
        ((Display*)user)->PlayAudio(filename, volume);
    });

    // load startup URI as main window
    if (!StrIsNullOrEmpty(mStartupUri.Get()))
    {
    ...
    mMainWindow->Init(display, mRenderContext, Max(samples, 1U), ppaa, lcd);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
BaseComponent* Application::FindNodeResource(const char* key, bool fullElementSearch) const
{
...
{
    return mOwner != 0 ? mOwner->FindNodeName(name) : 0;
}

////////////////////////////////////////////////////////////////////////////////////////////////////
ObjectWithNameScope Application::FindNodeNameAndScope(const char* name) const
{
    return mOwner != 0 ? mOwner->FindNodeNameAndScope(name) : ObjectWithNameScope { };
}
