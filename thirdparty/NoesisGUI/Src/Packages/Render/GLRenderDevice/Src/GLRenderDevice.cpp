...
GLRenderDevice::~GLRenderDevice()
{
    ...
    for (uint32_t i = 0; i < NS_COUNTOF(mPsShaders); i++)
    {
        if (mPsShaders[i] != 0)
        {
            DeleteShader(mPsShaders[i]);
        }
    }
...
#if NS_RENDERER_OPENGL
    #include "Shader.120.vert.h"
    #include "Shader.140.vert.h"
    #include "Shader.120.frag.h"
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::EnsureVsShader(uint32_t n)
{
    ...
    if (mVsShaders[n] == 0)
    {
        ...
        static_assert(NS_COUNTOF(mVsShaders) == NS_COUNTOF(shaders), "Size mismatch");

#if NS_RENDERER_OPENGL
        if (mGLVersion >= NS_GL_VER(3, 1))
        {
            const char* strings[] = { GLSLV("140"), shaders[n], Shader_140_vert };
            mVsShaders[n] = CompileShader(GL_VERTEX_SHADER, strings, 3);
        }
        else
        {
            const char* strings[] = { GLSLV("120"), shaders[n], Shader_120_vert };
            mVsShaders[n] = CompileShader(GL_VERTEX_SHADER, strings, 3);
        }
#else
        if (mGLVersion > NS_GL_VER(2, 0))
        {
            const char* strings[] = { GLSLV("300 es"), shaders[n], Shader_300es_vert };
            mVsShaders[n] = CompileShader(GL_VERTEX_SHADER, strings, 3);
        }
        else
        {
            const char* strings[] = { GLSLV("100"), shaders[n], Shader_100es_vert };
            mVsShaders[n] = CompileShader(GL_VERTEX_SHADER, strings, 3);
        }
#endif
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::EnsurePsShader(uint32_t n)
{
    NS_ASSERT(n < NS_COUNTOF(mPsShaders));
    ...
    if (mPsShaders[n] == 0)
    {
        ...
        static_assert(NS_COUNTOF(mPsShaders) == NS_COUNTOF(shaders), "Size mismatch");

#if NS_RENDERER_OPENGL
        if (mGLVersion >= NS_GL_VER(3,1))
        {
            const char* strings[] = { GLSLV("140"), shaders[n], Shader_140_frag };
            mPsShaders[n] = CompileShader(GL_FRAGMENT_SHADER, strings, 3);
        }
        else
        {
            const char* strings[] = { GLSLV("120"), shaders[n], Shader_120_frag };
            mPsShaders[n] = CompileShader(GL_FRAGMENT_SHADER, strings, 3);
        }
#else
        if (mGLVersion > NS_GL_VER(2, 0))
        {
            const char* strings[] = { GLSLV("300 es"), shaders[n], Shader_300es_frag };
            mPsShaders[n] = CompileShader(GL_FRAGMENT_SHADER, strings, 3);
        }
        else
        {
            const char* strings[] = { GLSLV("100"), shaders[n], Shader_100es_frag };
            mPsShaders[n] = CompileShader(GL_FRAGMENT_SHADER, strings, 3);
        }
#endif
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::EnsureProgram(uint32_t n)
{
    NS_ASSERT(n < NS_COUNTOF(mPrograms));

    if (mPrograms[n].object == 0)
    {
        struct Program
        {
            uint8_t format;
        ...
        switch (n)
        {
            case Shader::RGBA: p = { VFPos, 0, 0 }; break;
            case Shader::Mask: p = { VFPos, 0, 1 }; break;
            case Shader::Path_Solid: p = { VFPos|VFColor, 1, 2 }; break;
            case Shader::Path_Linear: p = { VFPos|VFTex0, 2, 3 }; break;
            case Shader::Path_Radial: p = { VFPos|VFTex0, 2, 4 }; break;
            case Shader::Path_Pattern: p = { VFPos|VFTex0, 2, 5 }; break;

            case Shader::PathAA_Solid: p = { VFPos|VFColor|VFCoverage, 3, 6 }; break;
            case Shader::PathAA_Linear: p = { VFPos|VFTex0|VFCoverage, 4, 7 }; break;
            case Shader::PathAA_Radial: p = { VFPos|VFTex0|VFCoverage, 4, 8 }; break;
            case Shader::PathAA_Pattern: p = { VFPos|VFTex0|VFCoverage, 4, 9 }; break;

            case Shader::SDF_Solid: p = { VFPos|VFColor|VFTex1, 9, 10 }; break;
            case Shader::SDF_Linear: p = { VFPos|VFTex0|VFTex1, 10, 11 }; break;
            case Shader::SDF_Radial: p = { VFPos|VFTex0|VFTex1, 10, 12 }; break;
            case Shader::SDF_Pattern: p = { VFPos|VFTex0|VFTex1, 10, 13 }; break;

            case Shader::Image_Opacity_Solid: p = { VFPos|VFColor|VFTex1, 5, 14 }; break;
            case Shader::Image_Opacity_Linear: p = { VFPos|VFTex0|VFTex1, 6, 15 }; break;
            case Shader::Image_Opacity_Radial: p = { VFPos|VFTex0|VFTex1, 6, 16 }; break;
            case Shader::Image_Opacity_Pattern: p = { VFPos|VFTex0|VFTex1, 6, 17 }; break;

            case Shader::Image_Shadow35V: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 18 }; break;
            case Shader::Image_Shadow63V: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 19 }; break;
            case Shader::Image_Shadow127V: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 20 }; break;

            case Shader::Image_Shadow35H_Solid: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 21 }; break;
            case Shader::Image_Shadow35H_Linear: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 22 }; break;
            case Shader::Image_Shadow35H_Radial: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 23 }; break;
            case Shader::Image_Shadow35H_Pattern: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 24 }; break;

            case Shader::Image_Shadow63H_Solid: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 25 }; break;
            case Shader::Image_Shadow63H_Linear: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 26 }; break;
            case Shader::Image_Shadow63H_Radial: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 27 }; break;
            case Shader::Image_Shadow63H_Pattern: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 28 }; break;

            case Shader::Image_Shadow127H_Solid: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 29 }; break;
            case Shader::Image_Shadow127H_Linear: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 30 }; break;
            case Shader::Image_Shadow127H_Radial: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 31 }; break;
            case Shader::Image_Shadow127H_Pattern: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 32 }; break;

            case Shader::Image_Blur35V: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 33 }; break;
            case Shader::Image_Blur63V: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 34 }; break;
            case Shader::Image_Blur127V: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 35 }; break;

            case Shader::Image_Blur35H_Solid: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 36 }; break;
            case Shader::Image_Blur35H_Linear: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 37 }; break;
            case Shader::Image_Blur35H_Radial: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 38 }; break;
            case Shader::Image_Blur35H_Pattern: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 39 }; break;

            case Shader::Image_Blur63H_Solid: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 40 }; break;
            case Shader::Image_Blur63H_Linear: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 41 }; break;
            case Shader::Image_Blur63H_Radial: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 42 }; break;
            case Shader::Image_Blur63H_Pattern:p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 43 }; break;

            case Shader::Image_Blur127H_Solid: p = { VFPos|VFColor|VFTex1|VFTex2, 7, 44 }; break;
            case Shader::Image_Blur127H_Linear: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 45 }; break;
            case Shader::Image_Blur127H_Radial: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 46 }; break;
            case Shader::Image_Blur127H_Pattern: p = { VFPos|VFTex0|VFTex1|VFTex2, 8, 47 }; break;

            default: NS_ASSERT_UNREACHABLE;
        }

        EnsureVsShader(p.vsIndex);
        EnsurePsShader(p.psIndex);
        ...
        GLuint po = LinkProgram(mVsShaders[p.vsIndex], mPsShaders[p.psIndex], attribs);
        ...
        mPrograms[n].projectionMtxLocation = glGetUniformLocation(po, "projectionMtx");
        mPrograms[n].projectionMtxHash = 0;

        mPrograms[n].textSizeLocation = glGetUniformLocation(po, "textSize");
        mPrograms[n].textPixelSizeLocation = glGetUniformLocation(po, "textPixelSize");
        mPrograms[n].textureSizeHash = 0;

        mPrograms[n].opacityLocation = glGetUniformLocation(po, "opacity");
        mPrograms[n].opacityHash = 0;

        mPrograms[n].radialGradLocation = glGetUniformLocation(po, "radialGrad");
        mPrograms[n].radialGradHash = 0;

        mPrograms[n].rgbaLocation = glGetUniformLocation(po, "rgba");
        mPrograms[n].rgbaHash = 0;

        mPrograms[n].effectParamsLocation = glGetUniformLocation(po, "effectParams");
        mPrograms[n].effectParamsHash = 0;

        mPrograms[n].object = po;
        mPrograms[n].vertexFormat = p.format;
    }
}
...
void GLRenderDevice::CreatePrograms()
{
    ...
    memset(mPsShaders, 0, sizeof(mPsShaders));
...
// having to use glGetProgramBinary or glProgramBinary
    ...
    EnsureProgram(Shader::RGBA);
    EnsureProgram(Shader::Mask);

    EnsureProgram(Shader::Path_Solid);
    EnsureProgram(Shader::Path_Linear);
    EnsureProgram(Shader::Path_Radial);
    EnsureProgram(Shader::Path_Pattern);

    EnsureProgram(Shader::PathAA_Solid);
    EnsureProgram(Shader::PathAA_Linear);
    EnsureProgram(Shader::PathAA_Radial);
    EnsureProgram(Shader::PathAA_Pattern);

    EnsureProgram(Shader::SDF_Solid);
    EnsureProgram(Shader::SDF_Linear);
    EnsureProgram(Shader::SDF_Radial);
    EnsureProgram(Shader::SDF_Pattern);

    EnsureProgram(Shader::Image_Opacity_Solid);
    EnsureProgram(Shader::Image_Opacity_Linear);
    EnsureProgram(Shader::Image_Opacity_Radial);
    EnsureProgram(Shader::Image_Opacity_Pattern);

    EnsureProgram(Shader::Image_Shadow35V);
    EnsureProgram(Shader::Image_Shadow63V);
    EnsureProgram(Shader::Image_Shadow127V);

    EnsureProgram(Shader::Image_Shadow35H_Solid);
    EnsureProgram(Shader::Image_Shadow35H_Linear);
    EnsureProgram(Shader::Image_Shadow35H_Radial);
    EnsureProgram(Shader::Image_Shadow35H_Pattern);

    EnsureProgram(Shader::Image_Shadow63H_Solid);
    EnsureProgram(Shader::Image_Shadow63H_Linear);
    EnsureProgram(Shader::Image_Shadow63H_Radial);
    EnsureProgram(Shader::Image_Shadow63H_Pattern);

    EnsureProgram(Shader::Image_Shadow127H_Solid);
    EnsureProgram(Shader::Image_Shadow127H_Linear);
    EnsureProgram(Shader::Image_Shadow127H_Radial);
    EnsureProgram(Shader::Image_Shadow127H_Pattern);

    EnsureProgram(Shader::Image_Blur35V);
    EnsureProgram(Shader::Image_Blur63V);
    EnsureProgram(Shader::Image_Blur127V);

    EnsureProgram(Shader::Image_Blur35H_Solid);
    EnsureProgram(Shader::Image_Blur35H_Linear);
    EnsureProgram(Shader::Image_Blur35H_Radial);
    EnsureProgram(Shader::Image_Blur35H_Pattern);

    EnsureProgram(Shader::Image_Blur63H_Solid);
    EnsureProgram(Shader::Image_Blur63H_Linear);
    EnsureProgram(Shader::Image_Blur63H_Radial);
    EnsureProgram(Shader::Image_Blur63H_Pattern);

    EnsureProgram(Shader::Image_Blur127H_Solid);
    EnsureProgram(Shader::Image_Blur127H_Linear);
    EnsureProgram(Shader::Image_Blur127H_Radial);
    EnsureProgram(Shader::Image_Blur127H_Pattern);
...
GLuint GLRenderDevice::CompileShader(GLenum type, const char** strings, uint32_t count) const
{
    ...
    if (status == 0)
    {
        ...
        NS_FATAL("Shader compilation failed: %s", error);
    }
...
GLuint GLRenderDevice::LinkProgram(GLuint vso, GLuint pso, const char** attribs) const
{
    ...
    if (status == 0)
    {
        ...
        NS_FATAL("Link program failed: %s", error);
    }
...
void GLRenderDevice::ActivateProgram(const Batch& batch)
{
    ...
    if (program.textSizeLocation != -1 || program.textPixelSizeLocation != -1)
    {
        // These uniforms are not used at the same time
        NS_ASSERT(program.textSizeLocation != program.textPixelSizeLocation);
        ...
        {
            if (program.textSizeLocation != -1)
            {
                V(glUniform2f(program.textSizeLocation, (float)width, (float)height));
            }
            else
            {
                V(glUniform2f(program.textPixelSizeLocation, 1.0f / width, 1.0f / height));
            }
